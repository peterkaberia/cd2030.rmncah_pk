  ...
  tagList(
    ...
    contentBody(
      tabBox(
        ...
        tabPanel(
          title = i18n$t("opt_ratio_hos_pop"),
          fluidRow(
            column(12, plotCustomOutput(ns('ratio_hos_pop'))),
            column(3, downloadButtonUI(ns('ratio_hos_pop_download')))
          )
        ),
...
healthSystemSubnationalServer <- function(id, cache, i18n) {
  ...
  moduleServer(
    ...
    module = function(input, output, session) {
      ...
      output$ratio_hos_pop <- renderCustomPlot({
        req(metric())
        plot(metric(), 'ratio_hos_pop', nat_metrics()$ratio_hos_pop, 2)
      })
